...
//  'use strict';
    ...
    function init() {
    	...
    	if (options.loading) {
			...
			$('.wrapper').append('<div class="parallax-loading"><div id="load"><div id="rond"><div id="test"></div></div><p id="loadNum"></p></div></div>');
        } else {
        	// 允许触摸滑动
            movePrevent = false;
        }
        ...
        $.fn.parallax.animClear = function(id){
            var thisDom = $('#' + id).find('[data-animation]').css({
                '-webkit-animation': 'none',
                'display': 'none'
            });
        };

        $.fn.parallax.animShow = function(){
            console.log('curPage', $animateDom);
            animShow();
        };
    ...
    function animShow() {
        ...
        $('.current [data-animation]').each(function(index, element){
            // if($(this).parent().hasClass("noanimate")){return;}   //  pei modify for  if parent is display:none; don't run the following and  when you need show ,you can  run  animShow() to show
            var $element    = $(element),
...
//              '-webkit-animation': $animation +' '+ $duration + 'ms ' + $timfunc + ' '+ $delay + 'ms both',
    ...
    window.animShow = animShow;   // pei modify  for you can  manual show  the animation
